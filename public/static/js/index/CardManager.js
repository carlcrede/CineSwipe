const CardManager = (() => {
  ...
  const checkAndRepopulate = async () => {
    if (document.hasFocus) {
      ...
      if (!items.length) {
        console.log(
          "out of items that match the filters in the current region."
        );
        // add a card that says "No more items to show"
        const noMoreItems = $(
        ...
        const instructions = $(
          `<div class="instructions">
                    ...
                    <p>Try changing the filters or providers</p>
                    </div>`
        );
        noMoreItems.append(instructions);
        $(".card-container").append(noMoreItems);
        clearInterval(cardInterval);
      }
      const cards = $(".card-container").children();
      if (cards.length < 10 && items) {
  ...
  (function insertFirstCard() {
    ...
    const likebtn = $(
      `<div class="begin" id="beginBtn">Begin swiping 👍</div>`
    );
