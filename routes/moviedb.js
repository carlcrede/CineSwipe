...
const fetchMovies = async (options) => {
  try {
    ...
    return Promise.all(
      moviesResponse.results.map(async (result) => {
...
const fetchTv = async (options) => {
  try {
    ...
    const tvList = await Promise.all(
      tvResponse.results.map(async (result) => {
...
const fetchProviders = async (watch_region) => {
  ...
  try {
    const [movieProvidersResponse, tvProvidersResponse] = await Promise.all([
      fetch(`${base_url}/movie${query}`),
      fetch(`${base_url}/tv${query}`),
    ]);
    const [movieProviders, tvProviders] = await Promise.all([
      movieProvidersResponse.json(),
      tvProvidersResponse.json(),
    ]);
...
const itemDetails = async (id, media_type) => {
  try {
    let details;
    if (media_type == "movie") {
      details = await moviedb.movieInfo({
        id: id,
        append_to_response: "watch/providers,videos",
      });
      details["media_type"] = "movie";
    } else {
      details = await moviedb.tvInfo({
        id: id,
        append_to_response: "watch/providers,videos",
      });
...
const fetchInitialItems = async (watch_region) => {
  ...
  const { movieOptions, tvOptions } = buildFilterOptions({
...
router.get("/initial/:watch_region", async (req, res, next) => {
  ...
  try {
    if (
      !cache.initialitems[watch_region] ||
      Date.now() - cache.initialitems[watch_region].time > 60 * 1000
    ) {
...
router.get("/ipinfo", async (req, res, next) => {
  // spoof some generic public ip
  const ip = req.headers["x-forwarded-for"] || req.socket.remoteAddress;
  const response = await fetch(`https://api.db-ip.com/v2/free/${ip}`);
  const ipinfo = await response.json();
  if (!response.ok) {
    res.send({ countryCode: "DK" });
  } else {
    const countryCode = extractCountryCode(ipinfo);
    res.send({ countryCode });
  }
});
//
const extractCountryCode = (ipinfo) => {
  if (ipinfo?.countryCode) {
    return ipinfo.countryCode;
  }
  if (typeof ipinfo === "object" && Object.keys(ipinfo).length > 1) {
    for (const key in ipinfo) {
      if (ipinfo[key]?.countryCode) {
        return ipinfo[key].countryCode;
      }
    }
  }
  return "DK";
